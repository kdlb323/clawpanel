...
export default {
  ...
  cleanupHowToDesc: _('1. 如果没有“已绑定”版本，先绑定要保留的安装；2. 对旧 PATH 残留，复制清理命令或在横幅中隔离；3. 刷新状态确认只剩目标版本生效。', '1. If no version is bound, bind the installation you want to keep. 2. For stale PATH entries, copy the cleanup command or quarantine them from the banner. 3. Refresh status to confirm only the target version remains active.', '1. 如果沒有「已綁定」版本，先綁定要保留的安裝；2. 對舊 PATH 殘留，複製清理命令或在橫幅中隔離；3. 重新整理狀態確認只剩目標版本生效。'),
